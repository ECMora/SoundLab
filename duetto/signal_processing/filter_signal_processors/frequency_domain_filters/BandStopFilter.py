...
class BandStopFilter(FilterSignalProcessor):
    """
    Class that executes a Band Stop filter on the frequencies of an AudioSignal
    """
    ...
    def filter(self,indexFrom=0, indexTo=-1):
        ...
        if self.checkIndexesOk(indexFrom, indexTo):
            ...
            frequency_cut_lower = int(round(self.frequency_cut_lower * indexFrecuency))
            frequency_cut_upper = int(round(self.frequency_cut_upper * indexFrecuency))

            size = len(data_frec)

            data_frec[frequency_cut_lower: frequency_cut_upper + 1] = complex(0, 0)
            data_frec[-frequency_cut_upper:size -frequency_cut_lower + 1] = complex(0, 0)

            if indexFrom == 0 and indexTo == len(self.signal):
                self.signal.data = np.array(np.real(ifft(data_frec)[0:indexTo - indexFrom]),
