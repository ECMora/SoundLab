...
class OneDimensionalElementsDetector(ElementsDetector):
    ...
    def detect(self,signal, indexFrom=0, indexTo=-1, threshold=0, decay=1,minSize=0,softfactor = 5,merge_factor=0,secondThreshold=0,
               threshold_spectral=95, pxx=[], freqs=[], bins=[], minsize_spectral=(0, 0),
            ...
            elements = [OscilogramElement(signal,c[0], c[1],number=i+1,threshold_spectral= threshold_spectral, pxx=pxx, freqs=freqs, bins=bins, minsize_spectral=minsize_spectral,
               merge_factor_spectral=merge_factor_spectral,location=location) for i,c in enumerate(self.one_dimensional_elements_detector(signal.data[indexFrom : indexTo],threshold, minSize=minSize, decay=decay, softfactor=softfactor, merge_factor=merge_factor,secondThreshold=secondThreshold))]
            self.oneDimensionalElements = [e for e in elements if True] #descartar elemento si no posee informacion espectral suficiente
