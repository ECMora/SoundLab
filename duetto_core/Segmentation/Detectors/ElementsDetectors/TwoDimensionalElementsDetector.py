...
class TwoDimensionalElementsDetector(Detector):
    ...
    def detect(self, signal, threshold, pxx, freqs, bins, minsize=(0, 0),
               merge_factor=(1,1),one_dimensional_parent=None,location=None):
        if(signal is None):
                return
        ...
        for i, j in gt_tresh_idx:
            if self._gt_tresh[i, j]:
                ...
                if regionBounds[1] - regionBounds[0] <= minsize[0] \
                    ...
                    rc = SpecgramElement(signal,pxx[regionBounds[0]: regionBounds[1],regionBounds[2]:regionBounds[3]],freqs,regionBounds[0],regionBounds[1],bins,regionBounds[2],regionBounds[3],number=elemIndex,one_dimensional_parent=one_dimensional_parent,location=location)
                    elemIndex+=1
