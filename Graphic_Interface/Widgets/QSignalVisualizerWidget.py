...
from Duetto_Core.Segmentation.Detectors.ElementsDetectors.IntervalElemDetector import IntervalElemDetector
...
from Duetto_Core.Segmentation.Detectors.ElementsDetectors.PeakToPeakElemDetector import PeakToPeakElemDetector
...
class QSignalVisualizerWidget(QWidget):
    ...
    def __init__(self, parent):
        ...
        self.elements_detectors = {'default': OneDimensionalElementsDetector(),
                                   'peak to peak': PeakToPeakElemDetector(),
                                   'intervals': IntervalElemDetector()}
    ...
    def _refresh(self, dataChanged, updateOscillogram, updateSpectrogram, partial):
        ...
        if self.visibleSpectrogram and updateSpectrogram and self.signalProcessor.signal \
           ...
           and self.mainCursor.max > self.mainCursor.min:
            ...
            #if not self.updatePxxMatrix and dataChanged:
            #    self.axesSpecgram.imageItem.setImage(numpy.transpose(self._Z),
            #        pos=(self._from_osc_to_spec(self.mainCursor.min - self.specgramSettings.NFFT) if partial else 0, 0))
            #else:
            self.axesSpecgram.imageItem.setImage(numpy.transpose(self._Z))
    ...
    def detectElements(self, **kwargs):
        self.clearCursors()
        detector = kwargs.pop('detector', 'default')
        elements_detector = self.elements_detectors[detector]
        kwargs['indexFrom'] = 0
        kwargs['indexTo'] = len(self.signalProcessor.signal.data)
        kwargs['specgramSettings'] = self.specgramSettings
        elements_detector.detect(self.signalProcessor.signal, **kwargs)

        if elements_detector.envelope is not None and len(elements_detector.envelope):
            self.envelopeCurve.setData(self.getTransformedEnvelope(elements_detector.envelope))
            self.setEnvelopeVisibility(True)
            self.axesOscilogram.setVisibleThreshold(True)

        for c in elements_detector.elements():
